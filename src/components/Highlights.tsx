...
function Highlights({ forecast, currentWeather }: any) {
  ...
  return (
    <div className="mx-auto md:w-3/12 sm:flex md:hidden md:h-screen relative xl:flex min-w-[24rem] md:bg-accentGrey overflow-hidden text-accentSecondary">
      {currentWeather ? (
        <div className="md:pl-4 mb-8 md:pr-6 w-full overflow-hidden">
          {/* location */}
          <div className="hidden h-28 md:flex items-center justify-center border-b">
          ...
          <div className="overflow-scroll space-y-4 min-h-[14rem] max-h-[19rem] no-scrollbar">
            {forecast?.list.slice(0, 7).map((item: any, idx: number) => (
              <div
                key={idx}
                className="bg-white border md:border-0 flex items-center justify-between px-4 py-2 rounded-xl"
              >
        ...
        <Skeleton className={skeletonVariants({ variant: "highlights" })} />
      )}
    </div>
  );
}
